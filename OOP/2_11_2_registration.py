...
#
# 1. Конструктор __init__ принимающий 2 аргумента (login, password). В конструкторе вы сохраняете переданные
...
#    1) Емайл(login) содержит хотя бы 1 символ "@". В случае ошибки выводим ValueError("Login must include at least one ' @ '")
#    2) Email(login) содержит хотя бы 1 символ " . ".В случае ошибки выводим ValueError("Login must include at least one ' . '")
...
#    1) Password является строкой(не список, словарь и т.д. ) в противном случае
# вызываем исключение TypeError("Пароль должен быть строкой")
#    2) Длина password должна быть от 5 до 11 символов, в противном случае
# вызывать исключение ValueError('Пароль должен быть длиннее 4 и меньше 12 символов')
#    3) Должен содержать хотя бы одну цифру. Для этого нужно в staticmethod создать функцию is_include_digit которая,
...
# В staticmethod создаем метод (is_include_all_register), который с помощью цикла проверяет элемента строчки на регистр.
# В случае ошибки выводит: ValueError('Пароль должен содержать хотя бы 2 заглавные буквы')
#    5) Строка password должна содержать только латинские символы. Импортируем библиотеку string,
# в staticmethod создаем метод(is_include_only_latin), которым проверяем,
# каждый элемент на наличие в string(проверка должна быть как в верхнем, так и нижнем регистре).
...
# Если значение совпадет со значением из файла, то в сеттер добавляем исключение и
# выводим ошибку: ValueError('Ваш пароль содержится в списке самых легких')
